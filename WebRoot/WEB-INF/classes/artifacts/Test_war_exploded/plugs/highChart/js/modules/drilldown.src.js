...
(function (H) {
	...
	Chart.prototype.addSeriesAsDrilldown = function (point,ddOptions) {
			var childdata = getChartEventData(point);
			// ddOptions = {name: '红3井',
		    //       data: [{name:'红1井',y:4,drilldown:'11'},{name:'大3樟树',y:6},{name:'毛泽3东旧居', y:2},{name:'列宁3大学', y:8}]
		    //     };
		if(parseInt(point.drilldown)<10) return;//click:对应的值说明,如果个位是1,表示图形有点击事件,如果十位是1表是有下钻事件
		for(var i in childdata){
			ddOptions = childdata[i];
	...
	H.Point.prototype.doDrilldown = function (_holdRedraw) {
		...
		/*
		 * 在页面中没有事先定义好下钻的数据时，seriesOptions为undefined，则addSeriesAsDrilldown就不会做了。
		 * 所以当动态获取下钻内容时seriesOptions设置为true
		 */
		 seriesOptions=true;
		if (seriesOptions) {
			if (_holdRedraw) {
